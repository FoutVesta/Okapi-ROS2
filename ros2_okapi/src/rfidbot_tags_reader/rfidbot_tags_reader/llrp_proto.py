...
def encode_C1G2Filter (par):
    ...
    data = struct.pack('!B', (par['T'] and 1 or 0) << 6)
    #if 'C1G2TagInventoryMask' in par:
    data += encode('C1G2TagInventoryMask')(par['C1G2TagInventoryMask'])
    #data += struct.pack('!H', par['C1G2TagInventoryStateAwareFilterAction'])
    #if 'C1G2TagInventorySUFilterAction' in par:
    data +=  encode('C1G2TagInventorySUFilterAction')(par['C1G2TagInventorySUFilterAction'])
    data = struct.pack(msg_header, msgtype,
            len(data) + struct.calcsize(msg_header)) + data
    return data
...
Message_struct['C1G2Filter'] = {
    ...
    'fields':[
        ...
        'C1G2TagInventorySUFilterAction',
    ],
...
def encode_C1G2Filter_C1G2TagInventoryMask(par):
    msgtype = Message_struct['C1G2TagInventoryMask']['type']
    msg_header = '!HH'

    data = struct.pack('!B',(par['MB'] and 1 or 0) << 6)
    data += struct.pack('!H', par['Pointer'])
    data += struct.pack('!H', par['MaskBitCount'])
    data += par['Tag mask']
...
Message_struct['C1G2TagInventoryMask'] = {
    ...
    'encode': encode_C1G2Filter_C1G2TagInventoryMask
}

# 16.3.1.2.1.1.3 C1G2TagInventoryStateUnawareFilterAction Parameter
def encode_C1G2Filter_C1G2TagInventorySUFilterAction(par):
    msgtype = Message_struct['C1G2TagInventorySUFilterAction']['type']
    msg_header = '!HH'
...
Message_struct['C1G2TagInventorySUFilterAction'] = {
    'type': 334,
    'fields':[
        'Action'
    ],
    'encode': encode_C1G2Filter_C1G2TagInventorySUFilterAction
}
...
class LLRPROSpec(dict):
    def __init__(self, llrpcli, msgid, priority=0, state='Disabled',
            antennas=(1,2), tx_power=91, duration_sec=None,
            report_every_n_tags=None, tag_content_selector={},
            session=0, tag_population=4,tag_filter=''): #session =2
        # Sanity checks
        if msgid <= 0:
        ...
        tagReportContentSelector = {
            'EnableROSpecID': True, #False, Jian modified
            'EnableSpecIndex': False,
            'EnableInventoryParameterSpecID': False,
            'EnableAntennaID': True,
            'EnableChannelIndex': True, #False, Jian modified
            'EnablePeakRRSI': True,
            'EnableFirstSeenTimestamp': True, #False, Jian modified
            'EnableLastSeenTimestamp': True,
            'EnableTagSeenCount': True,
            'EnableAccessSpecID': True, #False, Jian modified
        }
        ...
        self['ROSpec'] = {
            ...
            'ROBoundarySpec': {
                'ROSpecStartTrigger': {
                    ...
                    #Jian Adde, modify it to Null, only need the client to start it
            ...
            'AISpec': {
                ...
                'InventoryParameterSpec': {
                    ...
                    'InventoryParameterSpecID': 21,
                    'ProtocolID': AirProtocol['EPCGlobalClass1Gen2'],
                    'AntennaConfiguration': [],
                    #'AntennaConfiguration': [],C1G2 Filter
        ...
        for antid in antennas:
            if tag_filter != None and len(tag_filter) > 0:
                        ...
                        'C1G2InventoryCommand': {
                            ...
                            'C1G2Filter': {
                                ...
                                'C1G2TagInventoryMask': {
                                    'MB':1 , 
                                    'Pointer':0x20,
                                    'MaskBitCount':len(tag_filter)*4,
                                    'Tag mask':tag_filter.decode("hex")
                                    },
                                'C1G2TagInventorySUFilterAction':{
                                    'Action':0,},
                            },
