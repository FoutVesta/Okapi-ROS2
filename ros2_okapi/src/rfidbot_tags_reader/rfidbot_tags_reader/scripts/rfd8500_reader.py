...
class tagsRFD8500Reader():
    def __init__(self):
        rospy.init_node("rfid_tags_reader_RFD8500") #,log_level=rospy.INFO)
        self.nodename = rospy.get_name()
        rospy.logwarn("%s,%s started" ,"rfid_tags_reader_RFD8500", self.nodename)
        
        self.tags_pub = rospy.Publisher('rfid_tags', tagReader,queue_size=10)

        rospy.Subscriber('/set_rfd8500', Int16, self.setRFD8500Callback)
        rospy.Subscriber('/set_rfd8500_power', Int16, self.setRFD8500PowerCallback)
        ...
        #get default param
        self.defaultSession = int(rospy.get_param('~session',0))
        self.defaultTxPower = int(rospy.get_param('~txpower',130))
        rospy.logwarn("default session %d, txpower %d",self.defaultSession,self.defaultTxPower)
    ...
    def setRFD8500Callback(self,msg):   
        cmd = msg.data
        rospy.logwarn('receive cmd %d',cmd)
    ...
    def setRFD8500PowerCallback(self,msg):
        power  = msg.data
        rospy.logwarn('receive power %d',power)
        if power<=10:
            rospy.logwarn("set power failed!the power is invalid!")
            return
        if self.isInvStart:
            self.stopInventory()
        time.sleep(1)
    ...
    def getBattery(self):
        ...
        * ouput: print the battery information
        '''
        rospy.logwarn("getBattery")
        if self.gaugeSocket == None:
            rospy.logwarn("get battery info failed! the gaugeSocket is None")
            return
        ...
        rospy.logwarn("get the battery infor:%s",rec)
    ...
    def blueZConnect(self):
        ...
        * ouput: put socket objec to elf.gaugeSocket
        '''
        while(True):  
            rospy.logwarn("please push the trigger of RFD8500")
            try:
    ...
    def blueZdisconnect(self):
        ...
        * ouput: none
        '''
        if self.gaugeSocket == None:
            rospy.logwarn("blueZdisconnect failed! the gaugeSocket is None")
            return
        self.stopInventory()
    ...
    def readerConnect(self):
        ...
        * ouput: put socket objec to elf.gaugeSocket
        '''
        if self.gaugeSocket == None:
            rospy.logwarn("readerConnect failed! the gaugeSocket is None")
            return
        try:
            ...
            rospy.logwarn( "Caught BluetoothError: %s", error)
            time.sleep(5)
            pass
    ...
    def IsSessionIDSet(self,rec):
        ...
        * ouput: return Ture is connected, otherwise return False
        '''
        rospy.logwarn('IsSessionIDSet %s',rec)
        cmdFb = self.parseCommandFeedBack(rec)
        if cmdFb == None:
            return False
        rospy.logwarn('IsSessionIDSet:%s',cmdFb)
        for item in cmdFb:
    ...
    def setInvSession(self,sessionID):
        ...
        * ouput: None
        '''
        rospy.logwarn("setInvSession")
        if self.gaugeSocket == None:
            rospy.logwarn("setInvSession failed! the gaugeSocket is None")
            return
        cmd = 'qp' + ' '+ '.i'+' '+ str(sessionID)+'\r\n'
        rospy.logwarn("cmd:%s",cmd)
        try:
            self.gaugeSocket.send(cmd)
            while True:
                rec = self.gaugeSocket.recv(1024)
    ...
    def isInvStarted(self):
        ...
        * ouput: return True if invent start, otherwise return False
        ...
        if cmdFb == None:
            return False
        rospy.logwarn('isInvStart:%s',cmdFb)
        for item in cmdFb:
            if 'inventory' in item['Command'] :#and "OK" in item['Status']:
                rospy.logwarn('isInvStart:OK')
                return True
        return False
    ...
    def startInventory(self,power=None):
        ...
        * ouput: send inventory as ROS msg
        '''
        if self.gaugeSocket == None:
            rospy.logwarn("startInventory failed! the gaugeSocket is None")
            return
        if power == None:
            cmd = INV_RFID
        else:
            cmd = 'inventory'+' '+'.batchmode'+' '+ 'disable'+' '+'.ez .ir .ik .ih '+'.ec '+'.power'+' '+ str(power)+'\r\n'
            rospy.logwarn("new command %s",cmd)
        self.isInvStart = False # to make sure we can got feedback in isInvStart() 
        try:
            rospy.logwarn("start inv new command %s",cmd)
            self.gaugeSocket.send(cmd)
            while True:
                if self.isInvStarted():
                    break
            self.isInvStart = True
            rospy.logwarn("start inv new command success")
        except bluetooth.btcommon.BluetoothError as error:
    ...
    def isInvStoped(self):
        ...
        * ouput: return True if inventory stop, otherwise return False
        '''
        rec = self.gaugeSocket.recv(1024)
        rospy.logwarn('isInvStoped %s',rec)
        cmdFb = self.parseCommandFeedBack(rec)
        if cmdFb == None:
            return False
        rospy.logwarn('isInvStoped:%s',cmdFb)
        for item in cmdFb:
    ...
    def stopInventory(self):
        ...
        * ouput: none
        '''
        if self.gaugeSocket == None:
            rospy.logwarn("stopInventory failed! the gaugeSocket is None")
            return
        try:
            self.isInvStart = False
            ...
            while True:
                if self.isInvStoped():
                    break
            rospy.logwarn("set end inventory success")
        except bluetooth.btcommon.BluetoothError as error:
            print("Caught BluetoothError: ", error)
            time.sleep(5)
            pass
    ...
    def isBeeperVolumeSeted(self):
        ...
        * ouput: return True if success, otherwise return False
        '''
        rec = self.gaugeSocket.recv(1024)
        rospy.logwarn('isBeeperVolumeSeted %s',rec)
        cmdFb = self.parseCommandFeedBack(rec)
        if cmdFb == None:
            return False
        rospy.logwarn('isBeeperVolumeSeted:%s',cmdFb)
        for item in cmdFb:
    ...
    def setBeeperVolume(self,volume):
        ...
        * ouput: none
        '''
        if self.gaugeSocket == None:
            rospy.logwarn("setBeeperVolume failed! the gaugeSocket is None")
            return
        ...
        try:
            ...
            while True:
                if self.isBeeperVolumeSeted():
                    break
            rospy.logwarn("set Beeper Volume success")
        except bluetooth.btcommon.BluetoothError as error:
            print("Caught BluetoothError: ", error)
            time.sleep(5)
            pass
