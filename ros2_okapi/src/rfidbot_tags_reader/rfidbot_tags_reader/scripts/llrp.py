...
'''
*File name: llrp.py
*Description:  LLRP main file,modified by the sllrup-master, change all the log into rospy one
*Author: Jian zhang
...
class LLRPMessage:
    ...
    def serialize (self):
        ...
        rospy.logdebug('serializing %s command',name)
        ...
        self.msgbytes = struct.pack(self.full_hdr_fmt,
                ...
                msgid) + data
        rospy.logdebug('serialized bytes: %s',hexlify(self.msgbytes))
        rospy.logdebug('done serializing %s command',name)
    ...
    def deserialize (self):
        ...
        try:
            ...
            rospy.logdebug('deserializing %s command',name)
        ...
        try:
            ...
            rospy.logdebug('done deserializing %s command',name)
        except LLRPError as e:
            rospy.logwarn('Problem with %s message format %s',name, e)
            return ''
        return ''
    ...
    def isSuccess (self):
        ...
        try:
            ...
            rospy.logerr('failed to parse status from %s: %s',msgName,
                        KE)
            return False
...
class LLRPClient (LineReceiver):
    ...
    def __init__ (self, factory,tagFilter = '', duration=None, report_every_n_tags=None,
            antennas=(1,2), tx_power=0, modulation=DEFAULT_MODULATION, tari=0,
            start_inventory=True, reset_on_connect=True,
            disconnect_when_done=True,
            tag_content_selector={},
            session=0, tag_population=4):
        ...
        if self.reset_on_connect:
            rospy.loginfo('will reset reader state on connect')
        self.disconnect_when_done = disconnect_when_done
        self.tag_content_selector = tag_content_selector
        if self.start_inventory:
            rospy.loginfo('will start inventory on connect')

        rospy.loginfo('using antennas: %s',self.antennas)
    ...
    def parseCapabilities (self, capdict):
        ...
        if max(self.antennas) > gdc['MaxNumberOfAntennaSupported']:
            ...
            rospy.logwarn('Invalid antenna set specified: requested=%s,'
                        ' available=%s.\nIgnoring invalid antennas.',reqd, avail)
            self.antennas = [ant for ant in self.antennas \
                            if ant <= gdc['MaxNumberOfAntennaSupported']]

        # check requested Tx power
        rospy.logdebug('requested tx_power: %s',self.tx_power)
        ...
        rospy.logdebug('tx_power_table: %s',self.tx_power_table)
        if self.tx_power == 0:
        ...
        rospy.logdebug('set tx_power: %s (%s dBm)',self.tx_power,
        ...
        rospy.loginfo('requested modulation: %s',self.modulation)
        ...
        if not self.reader_mode:
            ...
            rospy.logwarn('Could not find reader mode matching '\
        ...
        rospy.loginfo('using reader mode: %s',self.reader_mode)

    def processDeferreds (self, msgName, isSuccess):
        deferreds = self._deferreds[msgName]
        if not deferreds:
            return
        rospy.logdebug('running %s Deferreds for %s; ' \
    ...
    def handleMessage (self, lmsg):
        ...
        #rospy.logwarn('LLRPMessage received in state %s: %s',self.state, lmsg)
        
        msgName = lmsg.getName()
        rospy.logdebug('received message %s',msgName)
        #rospy.loginfo('the state is %s',self.state)
        lmsg.peername = self.peername

        # call per-message callbacks
        rospy.logdebug('starting message callbacks for %s',msgName)
        for fn in self._message_callbacks[msgName]:
            fn(lmsg)
        rospy.logdebug('done with message callbacks for %s',msgName)
        ...
        if msgName == 'RO_ACCESS_REPORT' and \
            ...
            rospy.logdebug('ignoring RO_ACCESS_REPORT because not inventorying')
            return
        rospy.logdebug('in handleMessage(%s), there are %s Deferreds',
        ...
        if self.state in (LLRPClient.STATE_DISCONNECTED,
            ...
            if msgName != 'READER_EVENT_NOTIFICATION':
                rospy.logerr('unexpected message {} while' \
                        ' connecting',msgName)
                return
            ...
            if not lmsg.isSuccess():
                ...
                rospy.logfatal('Could not start session on reader:%s',status)
                return
            ...
            rospy.loginfo('send_GET_READER_CAPABILITIES')
        ...
        # respond to this message by advancing to state CONNECTED.
            ...
            if msgName != 'GET_READER_CAPABILITIES_RESPONSE':
                rospy.logerr('unexpected response %s ' \
                        ' when getting capabilities',msgName)
                return

            if not lmsg.isSuccess():
                status = lmsg.msgdict[msgName]['LLRPStatus']['StatusCode']
                err = lmsg.msgdict[msgName]['LLRPStatus']['ErrorDescription']
                rospy.logfatal('Error %s getting capabilities: %s',status,err)
                return

            self.capabilities = lmsg.msgdict['GET_READER_CAPABILITIES_RESPONSE']
            rospy.logdebug('Capabilities: %s',pprint.pformat(self.capabilities))
            try:
                self.parseCapabilities(self.capabilities)
            except LLRPError as err:
                rospy.logfatal('capabilities mismatch: %s',err)
                raise err

            self.processDeferreds(msgName, lmsg.isSuccess())
            rospy.loginfo('startInventory')
            if self.reset_on_connect:
                rospy.logwarn("STATE_SENT_GET_CAPABILITIES, and stop rospec")
                d = self.stopPolitely(disconnect=False)
                rospy.logwarn('startInventory 1')
                if self.start_inventory:
                    rospy.logwarn('we will start inventory here')
                    d.addCallback(self.startInventory)
            elif self.start_inventory:
                rospy.logwarn('we will start inventory and access')
                self.startInventory()
                #self.startAccess()
        ...
        # advancing to state SENT_ENABLE_ROSPEC.
            ...
            if msgName != 'ADD_ROSPEC_RESPONSE':
                rospy.logerr('unexpected response %s ' \
                        ' when adding ROSpec',msgName)
                return

            if not lmsg.isSuccess():
                status = lmsg.msgdict[msgName]['LLRPStatus']['StatusCode']
                err = lmsg.msgdict[msgName]['LLRPStatus']['ErrorDescription']
                rospy.logfatal('Error %s adding ROSpec: %s',status, err)
                return

            self.processDeferreds(msgName, lmsg.isSuccess())
            rospy.loginfo('ADD_ROSPEC_RESPONSE')
        ...
        # DISABLE_ROSPEC_RESPONSE.  advance to state PAUSED.
            ...
            if not lmsg.isSuccess():
                rospy.logwarn('%s,the lmsg is %s',msgName,lmsg)
                status = lmsg.msgdict[msgName]['LLRPStatus']['StatusCode']
                err = lmsg.msgdict[msgName]['LLRPStatus']['ErrorDescription']
                rospy.logerr('DISABLE_ROSPEC failed with status %s: %s' \
                        ,status, err)
                rospy.logfatal('Error %s disabling ROSpec: %s',status, err)
                return
        ...
        # ROSpec and advancing to state INVENTORYING.
            ...
            if msgName != 'ENABLE_ROSPEC_RESPONSE':
                rospy.logerr('unexpected response %s ' \
                        ' when enabling ROSpec',msgName)

            if not lmsg.isSuccess():
                status = lmsg.msgdict[msgName]['LLRPStatus']['StatusCode']
                err = lmsg.msgdict[msgName]['LLRPStatus']['ErrorDescription']
                rospy.logerr('ENABLE_ROSPEC failed with status %s: %s' \
                        ,status, err)
                rospy.logfatal('Error %s enabling ROSpec: %s',status, err)
                return

            self.processDeferreds(msgName, lmsg.isSuccess())
            #JIAN added
            self.send_START_ROSPEC()
            rospy.loginfo('ENABLE_ROSPEC_RESPONSE,send_START_ROSPEC')

        elif self.state == LLRPClient.STATE_INVENTORYING:
            #Jian added, add the START_ROSPEC_RESPONSE
            if msgName not in ('RO_ACCESS_REPORT', 'READER_EVENT_NOTIFICATION',
                ...
                rospy.logerr('unexpected message %s while' \
                        ' inventorying',msgName)
                return
            ...
            if msgName != 'DELETE_ACCESSSPEC_RESPONSE':
                rospy.logerr('unexpected response %s ' \
                        ' when deleting AccessSpec',msgName)

            self.processDeferreds(msgName, lmsg.isSuccess())
            
            rospy.loginfo('DELETE_ACCESSSPEC_RESPONSE')

        elif self.state == LLRPClient.STATE_SENT_DELETE_ROSPEC:
            if msgName != 'DELETE_ROSPEC_RESPONSE':
                rospy.logerr('unexpected response %s ' \
                        ' when deleting ROSpec',msgName)

            if lmsg.isSuccess():
                rospy.loginfo('reader finished inventory')
                ...
                status = lmsg.msgdict[msgName]['LLRPStatus']['StatusCode']
                err = lmsg.msgdict[msgName]['LLRPStatus']['ErrorDescription']
                rospy.logerr('DELETE_ROSPEC failed with status %s: %s',status, err)
            ...
            rospy.loginfo('DELETE_ROSPEC_RESPONSE')

        else:
            rospy.logwarn('message %s received in unknown state!',msgName)

        if self._deferreds[msgName]:
            rospy.logerr('there should NOT be Deferreds left for %s,' \
                    ' but there are!',msgName)


    def rawDataReceived (self, data):
        rospy.logdebug('got %s bytes from reader: %s',len(data),data.encode('hex'))
        ...
        while data:
            ...
            if len(data) >= LLRPMessage.full_hdr_len:
                ...
                rospy.logwarn('Too few bytes (%s) to unpack LLRP message' \
                        'header',len(data))
                self.partialData = data
            ...
            rospy.logdebug('expect %s bytes (have %s)',msg_len, len(data))

            if len(data) < msg_len:
                # got too few bytes
                rospy.loginfo('got too few bytes')
                self.partialData = data
                ...
                try:
                    ...
                    rospy.logwarn('Failed to decode  LLRPMessage: {}.  ' \
                            'Will not decode {} remaining bytes'.format(err,
                                len(data)))
                    break

    def panic (self, failure, *args):
        rospy.logerr('panic(): %s',args)
        rospy.logerr(failure.getErrorMessage())
        rospy.logerr(failure.getTraceback())

    def send_START_ROSPEC(self):
        rospy.loginfo('send START_ROSPEC')
    ...
    def send_ENABLE_ROSPEC (self, _, rospec, onCompletion):
        ...
        rospy.logwarn('send_ENABLE_ROSPEC')
    ...
    def send_ADD_ACCESSSPEC (self, accessSpec, onCompletion):
        ...
        rospy.loginfo('send_ADD_ACCESSSPEC')
    ...
    def send_ENABLE_ACCESSSPEC (self, _, accessSpecID, onCompletion=None):
        ...
        rospy.logeinfo('send_ENABLE_ACCESSSPEC')
        if onCompletion:
    ...
    def startAccess (self, readWords=None, writeWords=None, target = None,
        ...
        if not target:
            target = {
                ...
                'Pointer' : 0,
                'MaskBitCount': 0,
                'TagMask': '303404d4c80ff56d07a4b412',
                'DataBitCount': 0,
                'TagData': ''
            }
    ...
    def startInventory (self, *args):
        #rospy.logwarn('start invetory!')
        """Add a ROSpec to the reader and enable it."""
        if self.state == LLRPClient.STATE_INVENTORYING:
            rospy.logwarn('ignoring startInventory() while already inventorying')
            return None
        ...
        #the deferred will set the state into STATE_INVENTORYING
    ...
    def getROSpec (self):
        ...
        self.rospec = LLRPROSpec(self, 1, duration_sec=self.duration,
                            ...
                            tag_filter = self.tagFilter)
        #rospy.logwarn('ROSpec: %s',self.rospec)
        rospy.logwarn('the new power %s',self.tx_power)
        rospy.logwarn('the session %s',self.session)
        return self.rospec
    ...
    def stopPolitely (self, disconnect=False):
        ...
        rospy.loginfo('stopping politely')
        if disconnect:
            rospy.loginfo('will disconnect when stopped')
    ...
    def pause (self, duration_seconds=0):
        ...
        rospy.loginfo('pausing for %s seconds',duration_seconds)
    ...
    def resumeAndStartNewRosPEC(self):
        self.startInventory()
        
    def resume (self):
        rospy.logwarn(' resume() ')
        if self.state in (LLRPClient.STATE_CONNECTED,
                LLRPClient.STATE_DISCONNECTED):
            rospy.logwarn(' resume here! ')
            self.startInventory()
            return
        
        if self.state != LLRPClient.STATE_PAUSED:
            rospy.logwarn('cannot resume() if not paused; ignoring')
            return None 
        rospec = self.getROSpec()['ROSpec']
        rospy.logwarn("the new rospec %s",rospy.logwarn)
    ...
    def setActiveAntennas(self,actAntennas):
        '''
        * description: set the active antennas to reader
        * input: actAntennas, the new active atennas (1,3)
        '''
        self.antennas = actAntennas
...
class LLRPClientFactory (ClientFactory):
    def __init__ (self, onFinish=None, reconnect=False,
        ...
        self._message_callbacks = defaultdict(list) #defaultdict(list): generate the default message call back
    ...
    def addTagReportCallback (self, cb):
        rospy.loginfo('add tag reprot call back')
        self._message_callbacks['RO_ACCESS_REPORT'].append(cb)

    #Jian added
    def addEnableAccessSpecResCallback(self,cb):
        rospy.loginfo('add Enable AccessSpecRes call back')
        self._message_callbacks['ENABLE_ACCESSSPEC_RESPONSE'].append(cb)

    def buildProtocol(self, _):
        proto = LLRPClient(factory=self,tagFilter = self.tagFilter, **self.client_args)
    ...
    def clientConnectionLost(self, connector, reason):
        rospy.loginfo('lost connection: %s',reason.getErrorMessage())
    ...
    def clientConnectionFailed(self, connector, reason):
        rospy.loginfo('connection failed: %s',reason.getErrorMessage())
    ...
    #jian add for filter
    def resumeAndStartNewRosPECInventory(self):
        for proto in self.protocols:
            proto.resumeAndStartNewRosPEC()
            
    #jian add for filter        
    def pauseAndDeleteRosPECInventory(self):
        for proto in self.protocols:
            proto.pauseAndDeleteRosPEC()

    def politeShutdown (self):
        """Stop inventory on all connected readers."""
        protoDeferreds = []
        for proto in self.protocols:
            protoDeferreds.append(proto.stopPolitely(disconnect=False))#True))
    ...
    def setTagFilter(self,tagFilter):
        self.tagFilter = tagFilter
        for proto in self.protocols:
            proto.setFilter(self.tagFilter)

    #jian add
    def setNewTxPower(self,txPower):
        self.txPower = txPower
        if self.txPower == None:
            rospy.logwarn('the tx power is None can not set ')
            return
        for proto in self.protocols:
            proto.setTxPower(self.txPower)

    #jian add
    def setNewActiveAntennas(self, newAntennas):
        '''
        * description: set the new antenna to all proto
        * input: newAntennas,(1,3)
        '''
        self.newAntennas = newAntennas
        if self.newAntennas == None:
            rospy.logwarn('the antenns is None, can not set')
            return
        for proto in self.protocols:
            proto.setActiveAntennas(self.newAntennas)
